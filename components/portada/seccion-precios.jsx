...
function SeccionPrecios() {
  ...
  return <section id="pricing" className="relative py-32 lg:py-40 border-t border-foreground/10">
  ...
  >
          ...
          {isAnnual && <span className="ml-2 px-2 py-1 bg-foreground text-primary-foreground text-xs font-mono">
              Save 17%
            </span>}
        </div>
    ...
    /* Pricing Cards */
        ...
        <div className="grid md:grid-cols-3 gap-px bg-foreground/10">
          {plans.map((plan, idx) => <div
    key={plan.name}
    className={`relative p-8 lg:p-12 bg-background ${plan.popular ? "md:-my-4 md:py-12 lg:py-16 border-2 border-foreground" : ""}`}
  >
              {plan.popular && <span className="absolute -top-3 left-8 px-3 py-1 bg-foreground text-primary-foreground text-xs font-mono uppercase tracking-widest">
                  Most Popular
                </span>}
    ...
    /* Price */
              ...
              <div className="mb-8 pb-8 border-b border-foreground/10">
                {plan.price.monthly !== null ? <div className="flex items-baseline gap-2">
                    <span className="font-display text-5xl lg:text-6xl text-foreground">
                      ${isAnnual ? plan.price.annual : plan.price.monthly}
                    </span>
                    <span className="text-muted-foreground">/month</span>
                  </div> : <span className="font-display text-4xl text-foreground">Custom</span>}
              </div>
