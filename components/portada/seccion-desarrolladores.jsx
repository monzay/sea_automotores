...
function SeccionDesarrolladores() {
  ...
  >
    ...
    key={feature.title}
    className={`transition-all duration-500 ${estaVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
  ...
  >
    ...
    /* Right: Code block */
          ...
          <div
    className={`lg:sticky lg:top-32 transition-all duration-700 delay-200 ${estaVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}
  >
    ...
    /* Tabs */
              ...
              <div className="flex items-center border-b border-foreground/10">
                {codeExamples.map((example, idx) => <button
    key={example.label}
    type="button"
    ...
    /* Code content */
              ...
              <div className="p-8 font-mono text-sm bg-foreground/[0.01] min-h-[220px]">
                <pre className="text-foreground/80">
                  {codeExamples[activeTab].code.split("\n").map((line, lineIndex) => <div
    key={`${activeTab}-${lineIndex}`}
    className="leading-loose dev-code-line"
    style={{ animationDelay: `${lineIndex * 80}ms` }}
  >
                      <span className="inline-flex">
                        {line.split("").map((char, charIndex) => <span
